...
local function gBits8()
    local F = BitXOR(Byte(ByteString, Pos, Pos), 165);
    Pos = Pos + 1;
    return F;
end;
...
local function gString(Len)
	...
	local FStr = {}
	for Idx = 1, #Str do
		FStr[Idx] = Char(BitXOR(Byte(Sub(Str, Idx, Idx)), 165))
	end
    ...
    local Chunk = 
	{
		...
		Lines
								...
								for Idx=1,gBits32() do Functions[Idx-1]=Deserialize();end;for Idx=1,gBits32() do 
									local Data1=BitXOR(gBits32(),115);
									local Data2=BitXOR(gBits32(),182); 
...
local function Wrap(Chunk, Upvalues, Env)
	...
	return function(...)
		...
		while true do
			...
			Enum		= Inst[1];if Enum <= 3 then if Enum <= 1 then if Enum == 0 then do return end;else do return end;end; elseif Enum > 2 then Stk[Inst[2]]=Env[Const[Inst[3]]];else Stk[Inst[2]]=Env[Const[Inst[3]]];end; elseif Enum <= 5 then if Enum == 4 then Stk[Inst[2]]=Const[Inst[3]];else local A=Inst[2];local Args={};local Edx=0;local Limit=A+Inst[3]-1;for Idx=A+1,Limit do Edx=Edx+1;Args[Edx]=Stk[Idx];end;Stk[A](Unpack(Args,1,Limit-A));Top=A;end; elseif Enum > 6 then local A=Inst[2];local Args={};local Edx=0;local Limit=A+Inst[3]-1;for Idx=A+1,Limit do Edx=Edx+1;Args[Edx]=Stk[Idx];end;Stk[A](Unpack(Args,1,Limit-A));Top=A;else Stk[Inst[2]]=Const[Inst[3]];end;
